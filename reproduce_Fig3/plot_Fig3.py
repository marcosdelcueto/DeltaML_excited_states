...
def do_plots(df,df_test):
    ...
    # plot test data
    axs[0].scatter(S1_ZINDO_test, S1_TDDFT_test, alpha=0.75,edgecolors='C1',color='C1', linewidth=0.5,s=10)
    # plot labels for contour lines
    axs[0].clabel(cset2, cset2.levels, inline=False, fontsize=0,inline_spacing=-3)
    # labels
    axs[0].annotate(u'Train: $r$ = %.2f, $rmse$ = %.3f eV' % (r,rmse), xy=(0.30,0.12), xycoords='axes fraction', size=18, color='C4')
    axs[0].annotate(u'Test:  $r$ = %.2f, $rmse$ = %.3f eV' % (r_test,rmse_test), xy=(0.30,0.05), xycoords='axes fraction', size=18, color='C1')
    axs[0].set_xlabel(r"$E^{S1}_{ZINDO}\, (eV)$",  size=23, labelpad=12)
    axs[0].set_ylabel(r"$E^{S1}_{TDDFT}\, (eV)$", size=23, labelpad=12)
    axs[0].set_xlim(xymin,xymax)
    axs[0].set_ylim(xymin,xymax)
    axs[0].plot(np.arange(0, 8, 0.1), np.arange(0, 8, 0.1), color="k", ls="--")
    axs[0].text(0.05,1.08,'A)',fontsize=28,weight='bold',ha='right', va='top',transform=axs[0].transAxes)
    ...
    # plot test data
    axs[1].scatter(S1_ML_test, S1_TDDFT_test, alpha=0.75,edgecolors='C1',color='C1', linewidth=0.5,s=10)
    # plot labels for contour lines
    ...
    cbar.ax.set_ylabel('Number of points', rotation=90,size=23)
    # labels
    axs[1].annotate(u'Train: $r$ = %.2f, $rmse$ = %.3f eV' % (r_ML,rmse_ML), xy=(0.30,0.12), xycoords='axes fraction', size=18, color='C4')
    axs[1].annotate(u'Test:  $r$ = %.2f, $rmse$ = %.3f eV' % (r_ML_test,rmse_ML_test), xy=(0.30,0.05), xycoords='axes fraction', size=18, color='C1')
    axs[1].set_xlabel(r"$E^{S1}_{ML}\, (eV)$",  size=23, labelpad=12)
    axs[1].set_xlim(xymin,xymax)
    axs[1].set_ylim(xymin,xymax)
    axs[1].plot(np.arange(0, 8, 0.1), np.arange(0, 8, 0.1), color="k", ls="--")
    axs[1].text(0.05,1.08,'B)',fontsize=28,weight='bold',ha='right', va='top',transform=axs[1].transAxes)

    # Font sizes ticks
    axs[0].set_xticklabels(np.arange(xymin, xymax+xytick, xytick),fontsize=18)
    axs[0].set_yticklabels(np.arange(xymin, xymax+xytick, xytick),fontsize=18)
    axs[1].set_xticklabels(np.arange(xymin, xymax+xytick, xytick),fontsize=18)
