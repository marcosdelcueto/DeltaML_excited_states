...
def main():
    ...
    separate_test = True
    # set db file names
    train_csv_file = 'database/train_data.csv'
    test_csv_file  = 'database/test_data.csv'
    xcols = ['fingerprint','RDF']
    # Preprocess
    df = pd.read_csv(train_csv_file)
    df_test = pd.read_csv(test_csv_file)
    X,y,X_test,y_test,E_lin,E_lin_test = preprocess(df,df_test,xcols,separate_test)
    print('Preprocessing done')
    # ML
    rms = func_ML(X,y,X_test,y_test,E_lin,E_lin_test,separate_test,gamma_fp,gamma_rdf,regularization)
    return
...
def preprocess(df,df_test,xcols,separate_test):
    ...
    E_lin: np.array
        array with values of linear regression for train data
    E_lin_test: np.array
        array with values of linear regression for test data
    '''
    ...
    return X,y,X_test,y_test,E_lin,E_lin_test

def func_ML(X,y,X_test,y_test,E_lin,E_lin_test,separate_test,gamma_fp,gamma_rdf,regularization):
    '''
    Parameters
    ----------
    X: np.array
    ...
    if separate_test == False:
        ...
        y_real = y
        # Calculate S1_ml as linear + Delta_E_pred
        S1_tddft=[]
        S1_ml=[]
        for i in range(len(y_real)):
            S1_tddft.append(float(E_lin[i]+y_real[i]))
            S1_ml.append(float(E_lin[i]+y_predicted[i]))
        S1_tddft=np.array(S1_tddft)
        S1_ml=np.array(S1_ml)
        # calculate metrics
        r,_   = pearsonr(S1_tddft,S1_ml)
        rho,_ = spearmanr(S1_tddft,S1_ml)
        rms  = sqrt(mean_squared_error(S1_tddft,S1_ml))
        errors = abs(S1_tddft - S1_ml)
    ...
    # Predict test set
        ...
        S1_ml=[]
        for i in range(len(y_real)):
            S1_tddft.append(float(E_lin_test[i]+y_real[i]))
            S1_ml.append(float(E_lin_test[i]+y_predicted[i]))
        S1_tddft=np.array(S1_tddft)
        S1_ml=np.array(S1_ml)
        # calculate metrics
        r,_   = pearsonr(S1_tddft,S1_ml)
